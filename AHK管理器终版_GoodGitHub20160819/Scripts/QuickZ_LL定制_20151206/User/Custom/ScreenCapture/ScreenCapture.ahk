...
参数nQuality只有当sFileTo为JPG时适用，用此参数设置需要的质量等级，范围为0-100之间的整数。
4) If aRect is 0/1/2/3, captures the entire desktop/active window/active client area/active monitor.
参数aRect为0/1/2/3时，分别捕捉整个桌面/活动窗口/活动客户工作区/活动监视器。
...
参数aRect可为由逗号分隔的坐标值。e.g., "Left, Top, Right, Bottom" or "Left, Top, Right, Bottom, Width_Zoomed, Height_Zoomed".
...
;~ Convert("c:\bak\MY_Smail\screen.bmp", "c:\bak\MY_Smail\screen.jpg")
;~ Return
...
Convert(sFileFr = "", sFileTo = "", nQuality = "")
{
	...
	If Not	hGdiPlus := DllCall("LoadLibraryW", "str", "gdiplus.dll")
