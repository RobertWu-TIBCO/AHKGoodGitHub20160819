...
GlobalMenu:
	...
	AddToGlobalMenu("FuckListFiles","FuckListFiles")
	...
	AddToGlobalMenu("GlobalLavelSubMenu","GlobalLavelSubMenu")
...
GlobalMenuToLabel_var:="GlobalMenu"	
gosub GlobalMenuToLabel
return
...
GlobalMenuToLabel:
    ...
    for index, element in g_GlobalMenuItems
    {
        if (element[3] == "" || lastWindowClass == element[3])
        {
            Menu, % GlobalMenuToLabel_var, Add, % element[1], % element[2]
        }
    }

    Menu, % GlobalMenuToLabel_var, Show
    ;Menu, GlobalMenu, DeleteAll
return
...
Menu %menuName%, Add, %fileName%, %action%
...
Menu %menuName%, Add, %menuName%, Menu_Tray_Exit
Menu %menuName%, Add, %folderName%, %action%
Menu %menuName%, ToggleEnable, %menuName%
Menu %menuName%, Default, %menuName%
scriptCount = 0
...
Menu scripts_edit, Add, F:\Program Files\AutoHotkey\Scripts\chm, txt_edit
...
Menu txt_edit, Add, F:\Program Files\AutoHotkey\Scripts\AHK管理器【终版】\FromAutoHotkeyScriptFolder\Good\ListViewFolder\folderLinkFiles.xml, txt_edit
...
Menu folder_edit, Add, F:\Program Files\AutoHotkey\Scripts\AHK管理器【终版】\FromAutoHotkeyScriptFolder\Good\ListViewFolder\folderLink.xml, txt_edit
...
Menu RecentFolder, Add, F:\Program Files\AutoHotkey\Scripts\AHK管理器【终版】\FromAutoHotkeyScriptFolder\Good\ListViewFolder\RecentLink.xml, txt_edit
...
Menu workspace, Add, F:\Program Files\AutoHotkey\Scripts\AHK管理器【终版】\FromAutoHotkeyScriptFolder\Good\ListViewFolder\ecspace.xml, txt_edit
...
Menu openfiles_edit, Add, F:\Program Files\AutoHotkey\Scripts\AHK管理器【终版】\FromAutoHotkeyScriptFolder\Good\ListViewFolder\folderLinkOpenFiles.xml, txt_edit
...
Menu ahk_edit, Add, F:\Program Files\AutoHotkey\Scripts\AHK管理器【终版】\FromAutoHotkeyScriptFolder\Good\ListViewFolder\AllAhkLearn.xml, txt_edit
...
OpenWithAHK32:
    ...
    ;StringReplace, title, title, - Notepad++ [Administrator], , All
    StringReplace, title, title, - Notepad++, , All
...
1RButton:
;RButton & 1:: ; this makes ff unable to use gesture
;1 & RButton:: ; this makes you unable to type the Alt key simble
gosub ChmMenu
return
...
AddToGlobalSubMenu(name, subname,label,winClass := "")
{
    if (g_GlobalSubMenuItems == "")
    {
        g_GlobalSubMenuItems := Object()
    }

    g_GlobalSubMenuItems.Push([name,subname, label,winClass])
}

FuckListFiles:
	AddToGlobalSubMenu("FuckListFiles","RecentFolder","RecentFolder")
	AddToGlobalSubMenu("FuckListFiles","FuckAHK","FuckAHK")
	AddToGlobalSubMenu("FuckListFiles","RunFolderList_DirFav","RunFolderList")
	AddToGlobalSubMenu("FuckListFiles","Ahk Learn(&5)`tWin+ 5","AhkOpenMenu")
	AddToGlobalSubMenu("FuckListFiles","Ahk Open文档(&4)`tWin+ 4","OpenFileMenu")
	AddToGlobalSubMenu("FuckListFiles","Eclipse Workspace","EcSpace")
	gosub SetMenu
return

GlobalSubMenu:
AddToGlobalSubMenu("GlobalSubMenu","帮助文档(&1)`t","ChmMenu")
AddToGlobalSubMenu("GlobalSubMenu","常用文档(&2)`t","FileMenu") 
AddToGlobalSubMenu("GlobalSubMenu","常用路劲(&3)`t","FolderMenu")
gosub SetMenu
return

GlobalLavelSubMenu:
AddToGlobalSubMenu("GlobalLavelSubMenu","GlobalFolderLabel_RecentLink","GlobalFolderLabel_RecentLink")
AddToGlobalSubMenu("GlobalLavelSubMenu","GlobalLabelAnyFolderLnk_chm","GlobalLabelAnyFolderLnk_chm")
AddToGlobalSubMenu("GlobalLavelSubMenu","GlobalLabelAnyFolderLnk_suffix_chm","GlobalLabelAnyFolderLnk_suffix_chm")
gosub SetMenu
return

CurrentFileOperation:
AddToGlobalSubMenu("CurrentFileOperation","测试当前脚本", "OpenWithAHK", "Notepad++")
AddToGlobalSubMenu("CurrentFileOperation","测试当前脚本A32", "OpenWithAHK32", "Notepad++")
AddToGlobalSubMenu("CurrentFileOperation","发送到火狐", "OpenWithFF", "Notepad++")
AddToGlobalSubMenu("CurrentFileOperation","发送文件到记事本", "OpenWithNotepad", "Notepad++")
AddToGlobalSubMenu("CurrentFileOperation","发送到Everything", "OpenWithEve", "Notepad++")
AddToGlobalSubMenu("CurrentFileOperation","测试每行","RunEachLine")
AddToGlobalSubMenu("CurrentFileOperation","测试每行","RunEachLine")
;Menu %MenuName%, Add, %SubMenuName%, %SubMenuLabel%
gosub SetMenu
return
...
SetMenu:
    ...
    for index, element in g_GlobalSubMenuItems
    {
	  if (element[4] == "" || lastWindowClass == element[4])
        {
            Menu, % element[1], Add, % element[2], % element[3]
		}
    }

    Menu,% element[1], Show
;	Menu, GlobalSubMenu, Show
;   Menu, GlobalSubMenu, DeleteAll
return